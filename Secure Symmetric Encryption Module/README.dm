...
What
attacker are you defending against?
...
considerados se encuentran:
 ...
 un atacante que obtiene acceso al
     almacenamiento donde se guardan los contenedores cifrados

 un atacante que intenta modificar el
     contenido del archivo cifrado

 un atacante que intenta modificar los
     metadatos del archivo

 un atacante que intenta descifrar el
     archivo sin la clave correcta
En todos estos casos, el
sistema debe garantizar que:

 el contenido del archivo no pueda ser
     leído sin la clave correcta

 cualquier modificación del archivo o
     metadatos sea detectada

 el descifrado falle si la autenticidad de
     los datos no es válida
...
![test_encrypt_decrypt](assets/img1.png)
test_llave_erronea:
![test_llave_erronea](assets/img2.png)
test_texto_modificado:
![test_texto_modificado](assets/img3.png)
test_metadata_modificada:
![test_metadata_modificada](assets/img4.png)
test_encriptaciones_multiples:
![test_encriptaciones_multiples](assets/img5.png)
Y finalmente se corrieron
todos los tests de forma conjunta:
![total_tests](assets/img6.png)
