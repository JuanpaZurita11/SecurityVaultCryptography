...
describe("D6 - TEST EXTRA", () => {
	...
	it("Zod module verifications", () => {
		...
		const container: SignContainer = cryptoModule.create_container(
			keyStore,
			"1234",
			"Juan",
			cipherObject,
		...
		expect(cryptoModule.verify_key_container_structure(keyStore)).toBe(
			true,
		);
	});
	...
	it("Multiple recipients", () => {
		...
		const cipherObject: CipherObject = {
			...
			recipients: [
				{
					username: "Alice",
					publicKey: b64ToBytes(aliceKeyStore.public_key),
				},
				{
					username: "Bob",
					publicKey: b64ToBytes(bobKeyStore.public_key),
				},
			],
		};
		...
		const decrytpOwner: Uint8Array = cryptoModule.decrypt_container(
			container,
			"Juan",
			ownerKeyStore,
			"12345",
		...
		const decrytpAlice: Uint8Array = cryptoModule.decrypt_container(
			container,
			"Alice",
			aliceKeyStore,
			"67890",
		...
		const decrytpBob: Uint8Array = cryptoModule.decrypt_container(
			container,
			"Bob",
			bobKeyStore,
			"1124",
	...
	it("Remove recipients and add recipients dynamically", () => {
		...
		let container: SignContainer = cryptoModule.create_container(
			ownerKeyStore,
			"12345",
			"Juan",
			cipherObject,
		);
		container = cryptoModule.remove_recipients_from_container(
			container,
			ownerKeyStore,
			"12345",
			["Alice"],
		);
		container = cryptoModule.add_recipients_to_container(
			container,
			ownerKeyStore,
			"12345",
			[
				{
					username: "Hank",
					publicKey: b64ToBytes(hankKeyStore.public_key),
				},
			],
		);
		...
		const callback = () => {
			try {
				return cryptoModule.decrypt_container(
					container,
					"Alice",
					aliceKeyStore,
					"67890",
					b64ToBytes(ownerKeyStore.public_key),
				);
			} catch (err) {
		...
		const decryptHank: Uint8Array = cryptoModule.decrypt_container(
			container,
			"Hank",
			hankKeyStore,
			"12345",
	...
	it("allow KeyStorage password change without breaking acces to the containers", () => {
		...
		aliceKeyStore = cryptoModule.update_keystorage_password(
			aliceKeyStore,
			"67890",
			"12345",
		);
		...
		const callback = () => {
			try {
				return cryptoModule.update_keystorage_password(
					aliceKeyStore,
					"985432",
					"67890",
				);
