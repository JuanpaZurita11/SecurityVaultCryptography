...
export interface SignedContainer extends EncryptionMetadata {
	cipherText: string;
	tag: string;
	signature: string;
	signerInfo: {
...
export class SignatureCryptoModule {
  ...
  */
	...
	encrypt_file(cipherObject: CipherObject) {
		...
		for (const recipient of cipherObject.recipients) {
			...
			const pubKey: Uint8Array = new Uint8Array(
				this.str2ab(atob(recipient.key)),
			);
	...
	decrypt_file(
		...
		recipientKeyWrap: KeyWrap,
	): Uint8Array {
		const privKey: Uint8Array = new Uint8Array(
			this.str2ab(atob(privateKeyBase64)),
		);
		const recipientXPriv = ed25519.utils.toMontgomerySecret(privKey);

		const ephimeralPub: Uint8Array = new Uint8Array(
			this.str2ab(atob(recipientKeyWrap.ephimeralPub)),
		);
		...
		const symmetric_key = xchacha20(
			derivedKey,
			new Uint8Array(this.str2ab(atob(recipientKeyWrap.wrapNonce))),
			new Uint8Array(this.str2ab(atob(recipientKeyWrap.wrappedKey))),
		);
	...
	generate_signature(
		...
		const TAG_BYTES = 16;
		const ciphertextBytes = cipherText.slice(0, -TAG_BYTES);
		const tagBytes = cipherText.slice(-TAG_BYTES);
		...
		const containerDump = new TextEncoder().encode(
		...
		return {
			...
			signerInfo: {
				...
				fingerprint: btoa(
					String.fromCharCode(
						...sha256(
							new Uint8Array(this.str2ab(atob(senderPublicKey))),
						),
					),
				),
			},
	...
	validate_signature(
		...
		if (
			!container.signerInfo ||
			typeof container.signerInfo.fingerprint !== "string"
		) {
			throw new Error(
				"Invalid container: missing or malformed signerInfo.",
			);
		}
		if (
			typeof container.signature !== "string" ||
			container.signature.length === 0
		) {
			throw new Error(
				"Invalid container: signature field is missing or empty.",
			);
		}
		if (!container.cipherText || !container.tag || !container.nonce) {
			throw new Error(
				"Invalid container: one or more required fields are missing.",
			);
		}

		const digest = new Uint8Array(
			this.str2ab(atob(container.signerInfo.fingerprint)),
		);
		...
		if (!equalBytes(digest, derivedFingerprint)) {
			throw new Error(
				"Fingerprint mismatch: The provided public key does not match the signature's signer info.",
			);
		}
		...
		const signatureBytes = new Uint8Array(
			this.str2ab(atob(container.signature)),
		);
		if (
			!ed25519.verify(
				signatureBytes,
			...
			throw new Error(
				"Signature verification failed: The signature does not match the message and / or public key.",
			);
		}
		return true;
	}
